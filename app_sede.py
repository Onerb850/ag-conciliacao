    ...
    if not df_vazio_pa.empty:
        ...
        with st.expander("🗑️ Selecionar Mapa para Exclusão", expanded=False):
            ...
            if c_btn.button("🗑️ Apagar este Mapa", type="primary", use_container_width=True):
                # CORREÇÃO: Forçando tipo string para que a comparação funcione sempre no Pandas vs Excel
                df_restante = df_vazio_pa[
                    ~((df_vazio_pa["Data"].astype(str) == str(del_data)) & (df_vazio_pa["Mapa"].astype(str) == str(del_mapa)))
                ]
    ...
    if not df_vazio_lote.empty:
        ...
        with st.expander("🗑️ Apagar um lote", expanded=False):
            ...
            if cdl3.button("🗑️ Apagar", type="primary", use_container_width=True, key="btn_del_lote"):
                # CORREÇÃO: Forçando a comparação como texto para ignorar formatação diferente do Excel
                df_restante_lote = df_vazio_lote[
                    ~((df_vazio_lote["Data"].astype(str) == str(del_data_lote)) & (df_vazio_lote["Mapas"].astype(str) == str(del_lote_chave)))
                ]
